...
import { dummyIssues, Issue } from '@/lib/dummyData';
import { TrendingUp, FileText, Calendar, Award } from 'lucide-react';

function IssueCard({ issue }: { issue: Issue }) {
  const totalSentiment = issue.sentiment.positive + issue.sentiment.neutral + issue.sentiment.negative;
  const negativePercent = ((issue.sentiment.negative / totalSentiment) * 100).toFixed(0);
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex-1 bg-gray-200 rounded-full h-2">
                <div
                  ...
                  style={{ width: `${(issue.sentiment.positive / totalSentiment) * 100}%` }}
                />
              </div>
              <div className="w-12 text-sm text-right">{issue.sentiment.positive}%</div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <div className="flex-1 bg-gray-200 rounded-full h-2">
                <div
                  ...
                  style={{ width: `${(issue.sentiment.neutral / totalSentiment) * 100}%` }}
                />
              </div>
              <div className="w-12 text-sm text-right">{issue.sentiment.neutral}%</div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <div className="flex-1 bg-gray-200 rounded-full h-2">
                <div
                  ...
                  style={{ width: `${(issue.sentiment.negative / totalSentiment) * 100}%` }}
                />
              </div>
              <div className="w-12 text-sm text-right font-semibold">{issue.sentiment.negative}%</div>
            </div>
          </div>
        </div>
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="professional-container py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600">실시간 입법 수요 분석 현황을 확인하세요</p>
        </div>
            ...
            <TabsContent key={period} value={period} className="space-y-6">
              ...
              {period !== 'daily' && dummyIssues[period].length > 0 && (
                <Card className="bg-gradient-to-br from-gray-900 to-gray-800 text-white">
                  <CardHeader>
                    <CardTitle className="text-white">1순위 법안 상세</CardTitle>
                    ...
                    {dummyIssues[period][0].relatedLaws[0] && (
                      <>
                        <div>
                          ...
                          <div className="flex items-center gap-2 mb-4">
                            ...
                            <span className="text-gray-300">
                              관련도 {dummyIssues[period][0].relatedLaws[0].relevanceScore}점
                            </span>
                          </div>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {dummyIssues[period][0].keywords.map((keyword, idx) => (
                              <Badge
                                key={idx}
                                variant="outline"
